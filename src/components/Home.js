...
const Loginform = (args) => {
  ...
  useEffect(() => {
    (async () => {
      ...
      await axiosConfig
        .get(`/holiday/view-working/${userId?.database}`)
        .then((res) => {
          // console.log(res?.data?.WorkingHours);
          if (res?.data?.WorkingHours?.length > 0) {
            setTiming(res?.data?.WorkingHours);
          }
        })
        .catch((err) => {
          console.log(err);
  ...
  const MarkAttendance = async (
    ...
    formdata.append("shift", JSON.stringify(shift));
